...
- **`discuss`** — pure conversation, no writes. If the user wants to talk through a component idea without committing, use that first, then `build-component` to actually ship.
- **`project-audit`** — periodic consistency scan across canon. Will catch drift between `STRUCTURE.md` and the actual filesystem if it happens.

The chain: `init-project` → `design-direction` (optional) → `build-component` (per atomic piece) → `marketing-brief` (once, end of project) → `build-page` (per page) → `design-check` (auto) → `project-audit` (periodic).
