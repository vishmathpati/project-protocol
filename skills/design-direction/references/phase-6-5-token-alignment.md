...
Always — between Phase 6 (user picks direction) and Phase 7 (write outputs).
Not skippable.
...
1. **Detect surface tier(s) for this project.** From `agents/BRAND.md` Product.Surfaces
   ...
   - marketing-only: type scale ratio 1.25 (or 1.333 for editorial brands),
     more accent saturation, display font MUST come from the editorial /
     expressive set (banned list applies — no Inter/Geist as display)
...
4. **Compare each token category against the locked direction**. For each, decide MATCH or MISMATCH:
   ...
   |---|---|---|
   | `font.display` | locked direction's display font == existing display | locked is different family, OR existing is on banned-from-display list |
...
The locked direction from Phase 6 carries an archetype (from Phase 2). Use
...
| Archetype | Display font (Phase 6.5 default — free where possible) | Body font | Mono |
|---|---|---|---|
...
- The display banned list in DESIGN.md template applies: Inter/Geist/Söhne/IBM
  Plex/SF Pro/Roboto/Space Grotesk/Open Sans/Public Sans/system-ui are NOT
  acceptable as display EXCEPT when the archetype is Hero or Everyman (where
  weight-contrast within one sans family is the chosen pairing strategy) or
  dashboard-only surface (where display rarely appears).
...
5. **Report alignment summary** to user. Three possible outcomes:
   ...
   - **All MATCH** → existing tokens still fit. Phase 7 writes only Overview + brand-specific DO NOT additions. Tell user: "your existing tokens still match the locked direction. No token changes needed."
   ...
   - **Full MISMATCH** → locked direction doesn't match any existing tokens (typical for first-time `design-direction` run, or major re-anchor). Propose full new token frontmatter.
...
7. **Preview the proposed token diff** before Phase 7 writes. Format:

```
Token alignment — comparing locked direction "Editorial Cream" vs existing DESIGN.md:

font.display:    Inter         →  Migra              [MISMATCH — Inter is banned from display]
...
8. **On user approval**, pass the new token values to Phase 7.
...
- **The locked direction is the source of truth** for token regeneration — not the user's gut. If the user wants different colors, they should adjust the direction in Phase 6, not override token-by-token.
...
- **Surface detection drives type-scale + display-font selection.** Dashboard
  ...
  1.25/1.333 + strict display banning per the template's banned: list.

---

## Output

A structured alignment object passed to Phase 7:
...
Phase 7 reads this and writes accordingly.
...
from this `design-direction` run. Only BRAND.md and DESIGN.md Overview will
update. If you wanted visual change, restart the skill and reconsider the
locked direction in Phase 6."
...
"display is too heavy", "dark mode is muddy"). Phase 6.5:
...
The direction itself might be wrong — want to step back to Phase 6 and
reconsider?"
