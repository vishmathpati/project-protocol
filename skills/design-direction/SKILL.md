...
Five concrete outputs, written to the project's `agents/` tier:
...
2. **`agents/DESIGN.md` Overview section** — the `[VERIFY — fill from BRAND.md or user direction]` placeholder is replaced with a one-paragraph identity statement that names tone, era, and references.
...
4. **`agents/DESIGN.md` token frontmatter** — colors, typography (display / body / mono families), spacing / radius / shadow scales, and `light_mode` + `dark_mode` blocks. Generated by Phase 6.5 and written by Phase 7 *only after explicit user approval* via the preview page. Never silent.
5. **`agents/preview/<direction-slug>-<date>.html`** — a self-contained HTML preview page rendering the proposed tokens. This is the visual approval surface — the user opens it in a browser before Phase 7 writes anything.
...
Write whatever Phase 6.5 returned approved: `agents/BRAND.md` (full), `agents/DESIGN.md` Overview, brand-specific DO NOT additions, **and token frontmatter (colors / typography / spacing / radius / shadow / light_mode / dark_mode) when Phase 6.5 returned approved values**. Per the rules in **What it produces** + **Compatibility** above.
...
Wrote:
  ...
  ✅ agents/DESIGN.md — token frontmatter (if Phase 6.5 returned approved values)
  ✅ agents/preview/<direction-slug>-<date>.html

Next step: run `npx @google/design.md lint agents/DESIGN.md` to sync DESIGN.md → global.css, then start UI work.
```
