...
9. **UI components — archetype-scoped (see `brain/DESIGN.md` `archetype:`).** For **`dashboard` / `app`** archetypes: shadcn-first when shadcn is in `package.json` — always use the shadcn primitive (`<Button>`, `<Input>`, `<Select>`, `<Dialog>`, `<Popover>`, `<Tooltip>`, `<Sheet>`, `<Drawer>`, `<Tabs>`, `<Accordion>`, `<DropdownMenu>`, `<Command>`, `<Toast>`, `<Alert>`, `<Checkbox>`, `<Radio>`, `<Switch>`, `<Slider>`, `<Textarea>`); never silently fall back to the native element because it's faster to type. For **`marketing` / `content` / `commerce`** archetypes: the *expressive* components (heroes, editorial bands, galleries, scroll sequences) are built **bespoke to the brand** — shadcn and community libraries (Aceternity, Magic UI) are craft *references*, never installed aesthetics; use shadcn only for genuine primitives (form controls, dialogs, menus). Either way — allowed native elements: `<a href>` for navigation, `<button type="submit">` inside a `<form>` when no styled `<Button>` is needed, `<input type="hidden">`. If a needed primitive doesn't exist in shadcn (e.g. date-range picker), stop and ask: build one in `components/ui/` following shadcn conventions, install a community block, or use another library.
...
- **DESIGN.md** — project tokens (+ `archetype:`, `motion:`, `icon:`). When this changes: pending UI work pauses until tokens validate.
- **TASTE.md** — learned-preferences ledger (confidence-scored). Fed by `save-session`; read by `build-page` / `design-check` so high-confidence preferences apply before the user re-asks. No cascade.
