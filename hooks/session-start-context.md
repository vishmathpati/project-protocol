...
2. **brain/STATUS.md** — current state, known bugs, next actions
3. **brain/STRUCTURE.md** — structural map of the project: surfaces present, component locations per tier, conventions (if present — created by `build-component` on first invocation)
4. **brain/BRAND.md** — product identity and what NOT to build (if it exists)
5. **brain/BRIEF.md** — every major decision made and why, including rejected options (if it exists)
6. **brain/docs/INDEX.md** — project map + agent dependency index (if it exists)

Before any UI work: also read **brain/FUNDAMENTALS.md** + **brain/DESIGN.md** if they exist.

Do NOT ask the user what the current state is if brain/STATUS.md exists. Read it.
...
1. **NEW standalone feature** — completely new capability, no existing code touches it
   → Add to brain/docs/INDEX.md Features section + create dependency map entry before session ends

2. **ADDITION to existing feature** — extending or modifying something that already exists
   → Read its dependency map entry in brain/docs/INDEX.md first
...
3. **UI CHANGE** — modifying how something looks or behaves visually
   → Check brain/docs/INDEX.md Critical Functions/Components for shared UI elements
...
## brain/WORKLOG.md discipline

After EVERY response where you changed a file, found a bug, made a decision, or tried
something that didn't work — append one line to brain/WORKLOG.md immediately:
...
Run the `save-session` skill before closing. It updates brain/CHANGELOG.md, brain/STATUS.md,
signs brain/BRIEF.md if decisions were made, and clears brain/WORKLOG.md.
