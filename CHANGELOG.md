...
### Fixed
- **Dashboard state and evidence safety.** The localhost dashboard is token-gated, same-origin for writes, serialized across concurrent submissions, resilient against post-start research-directory replacement, restricted to safe worktree-local evidence assets, and preserves submitted state across regeneration. Static HTML retains Export/Copy fallback without claiming synchronization.
- **Project Dashboard is now a decision interface, not a Markdown dump.** Canon views are bounded summaries; Research/Moodboard shows one concept and one site at a time with large grouped evidence, live links, capture-quality warnings, video-aware labels, and browser-local Keep/Maybe/Blend/Reject notes that never write canon.
- **UI Research waits for visual readiness and records video honestly.** Complete captures require settled fonts, decoded visible images, usable video metadata/frame, cleared loaders, and stable layout samples within a bounded wait. Video role, playback, delivery, responsive behavior, and reduced-motion fallback are first-class evidence; a captured video frame is not classified as an image concept.
- **Pinned-reference concept selection can no longer be skipped.** UI Research now distinguishes open discovery, provided-reference concept discovery, and explicitly selected teardown. A URL list, “discover no more sites,” locked BRAND/DESIGN direction, old moodboard, or existing teardown is never treated as human selection. Both concept-discovery paths generate/refresh the Project Dashboard and stop for a recorded human choice; Round 2 and research-grounded Build Page composition reject missing or pending selection records. Existing verified pinned-site evidence is reused before any revisit.
- **UI Research evidence integrity.** Provided-reference missions now write to the active worktree, classify live/partial/fallback/no-visual evidence, validate mobile captures by viewport and dimensions, filter framework/widget palette noise, recover unstable screenshot channels without looping, maintain per-site manifests, separate evidence/inference/gaps, and finish by saturation with an honest `WITH DOCUMENTED GAPS` verdict when remaining capture gaps cannot change convergence.
- **Roles now adapt to app-created worktrees.** Local vs Worktree is treated as an application/user checkout choice independent of CEO/worker/solo authority. Solo and CEO no longer reject or misclassify worktree sessions; local sessions receive one isolation recommendation; workers can continue locally only on a clean dedicated chapter branch; Recap describes every worktree as a real isolated checkout; and Handoff can resume committed work in a fresh worktree from its recorded branch/checkpoint while protecting dirty files left in the original checkout.
...
- **Private workshop and canon-attachment machinery.** Removed the repo-local `edit-plugin`, repository-identity private-canon resolver, Session Start attachment, and post-checkout workflow. Contributor canon and personal workshop skills belong outside the distributable public source; normal consumer worktree support is unchanged.
